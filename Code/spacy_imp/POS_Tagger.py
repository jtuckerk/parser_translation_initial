...
def train(nr_iter, examples):
    ...
    ``nr_iter`` iterations.
    '''
    print "THIS DOES GET CALLLEEEDDDDDDDDDDDDDDDDDDDDDDDDD"
    model = AveragedPerceptron()
    for i in range(nr_iter):
        random.shuffle(examples)
        for features, class_ in examples:
            scores, conf = model.predict(features)
...
# class Parser(object):
#     def __init__(self, load=False):
#         model_dir = os.path.dirname("tmp1")
...
# 
# 
# class PerceptronTagger2(object):
...
#                 tag, conf = self.model.predict(features)
#             tags.append(tag)
#             prev2 = prev; prev = tag
#         return tags
# 
